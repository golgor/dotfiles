...
- **manifest** — `~/.config/fnox/config.toml`, a tracked symlink into this repo:
  the list of secret *references* (env var name → Bitwarden item/field) plus the
  fnox `[daemon]` table. fnox reads it from every directory regardless of cwd, so
  it is fully machine-agnostic and tracked. It holds no values; the reference
  names themselves are still public.
- **fnox daemon** — fnox's in-memory secret cache, enabled by `[daemon]` in the
  manifest. Holds resolved values for its `idle_timeout` so new shells reuse
  them instead of re-resolving through rbw on every prompt. Replaces the old
  age-encrypted sync cache, which was redundant because rbw already self-caches.
- **keyring pinentry** — `rbw-pinentry-keyring`, selected in the tracked
  `~/.config/rbw/config.json`: rbw's pinentry that reads the Bitwarden master
  password from the GNOME keyring instead of prompting, so `rbw unlock` (and thus
  fnox) resolves silently, even for non-TTY callers.
- **machine-local state** — files that must exist on each machine but never in
  this repo: rbw's device_id and offline vault under `~/.local/share/rbw/`,
  kubeconfig, and gcloud auth.
...
- **host package vs mise tool** — host packages (`[bootstrap.packages]`) are
  pacman/AUR system packages like `rbw` and `libsecret`; mise tools (`[tools]`) are
...
- **fnox config is tracked; only rbw's vault is machine-local.** Dropping the
  ...
  declarative `~/.config/rbw/config.json` are tracked symlinks. Because fnox reads
  its config from every directory, the old `~/fnox.toml` symlink and its
  parent-directory walk are gone, which structurally closes #29. Only rbw's
  device_id and vault under `~/.local/share/rbw/` stay per-machine.
...
- System provisioning beyond the listed host packages — this is not full OS
  config management.
- Auth/credential state: kube tokens, gcloud auth, rbw's device_id and vault.
